...
try {
  ...
  try {
    ...
    assert.match(await page.locator("#statusMessage").textContent(), /Graph phrase executed/i);
    assert.match(await page.locator("#bloomExplanation").innerText(), /Acct 777/i);
    assert.match(await page.locator("#bloomExplanation").innerText(), /authorized visible projection|authorized visible relationships/i);
    await page.locator("#scenePreset").selectOption("financial-flow");
    await page.locator("#applyScenePreset").click();
    assert.match(await page.locator("#statusMessage").textContent(), /Scene preset applied/i);
    assert.ok(await page.locator("svg.graph-style-boardroom").count() >= 1);
    assert.match(await page.locator("#bloomExplanation").innerText(), /ledger-backed|Financial transaction/i);
    await page.locator("#bloomPhrase").fill("paths between Acct 100 and Acct 777");
    await page.locator("#runBloomPhrase").click();
    assert.match(await page.locator("#bloomExplanation").innerText(), /known-at cutoff|Cutoff/i);
    assert.match(await page.locator("#bloomExplanation").innerText(), /tx-004/i);
    assert.match(await page.locator("#chartRows").innerText(), /Path explanation/i);
